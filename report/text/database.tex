...
Up till now the rules could have several condition and action but late in the implementation of the Website we have decides to only allow one action and condition per rule to get a more user-friendly website, more about this decision can be found in section \vref{}\fixme{ref til rules website}. Most functions are still implemented on the first design so they can handle several conditions and actions. 
