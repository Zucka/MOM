...
However, a few function can only work with one condition and action per rules. An example is the function that checks whether a user can use a media, called $db\_rules\_user\_can\_turn\_device\_on$. If the implementation of rules on the website is change a new version of these functions need to be made.
