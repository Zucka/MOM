...
//Signal lights.
#define READYFT 0
#define LOGGEDINWITHU 1
#define DISCONNECTEDWU 2
#define LOOKINGFORCONN 3
...
boolean checkNoTimeLeft()
{
  ...
  Serial.println(F("timeLeft: "));
  Serial.println(timeLeft);
  Serial.println(F("TempLeft: "));
  Serial.println(tempLeft);
  Serial.println(F("millis - lasttime: "));
...
void changeSignalLight(int insl)
{
  if(insl != slstate)
  {
    digitalWrite(slr, LOW);  
...
void getStatus(void)
{
  ...
  else
  {
    ...
    {
      timeLeft = 3;
      changeSignalLight(DISCONNECTEDWU);
    }
  }
}
  ...
  if(client.connect(server, 80))
  {
    ...
    *(on+i) = '\0';
    ...
    if(strcmp(stat,"OK") == 0)
    {
      ...
      timeLeft = atoi(timeRemaining->valuestring);
      Serial.println(timeLeft);
      state = 1;
      digitalWrite(dvc, HIGH); //Turn on the device controlled.
      signalAcceptUser();
      changeSignalLight(LOGGEDINWITHU);
    }
    else
    {
...
{  
  ...
  else
  {
    Serial.println(F("Connection Failed"));
    state = 2;
    changeSignalLight(LOOKINGFORCONN);
  }
}
